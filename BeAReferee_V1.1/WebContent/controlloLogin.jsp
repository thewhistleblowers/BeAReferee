...
<%@ page
	import="AreaAmministrativa.*, AreaPersonale.*, AreaAssociativa.*, AreaTecnica.*, Sistema.*, java.sql.*,java.util.*"%>
...
<body>
	<%
		...
		String user = null;
		String name = null;
		String userid = request.getParameter("cm");
		...
		if (nomeRes.next()){
			...
			codici.add(nomeRes.getString("CM"));
		}
		if (rs.next()) {
			...
			if (cat.equalsIgnoreCase("Arbitro") && (arb.next())) {
				...
				boolean ok=model.controllaInd(userid);
				String k=""+ok;
	...
	%>
	<h5>
		Accesso non valido, categoria scelta errata. <a href='index.jsp'>Torna
			al LogIn</a>
	</h5>
	<%
		}

			if (cat.equalsIgnoreCase("Presidente") && (pres.next())) {
				user = "<a href=\"HomePagePersonale_presidente.jsp?CM=" + rs.getString("CM") + "\">";
				...
				boolean ok=model.controllaInd(codici.get(i));
				}
	...
	%>
	<h5>
		Accesso non valido, Username o Password errato/a. <a href='index.jsp'>Torna
			al LogIn</a>
	</h5>
