...
<body>
	<%
		...
		if (session.getAttribute("presidente") != null) {
			...
			if (indiRes == "" || civ == "" || citta == "" || reg == "" || t == "" || email == "") {
				k = true;
				out.print("no");
			}
			...
			if (!k) {
				...
				ResultSet rs;
				ResultSet ar;
				int i = st.executeUpdate("update associato set Num_Telefono=" + telefono + ", Indirizzo='" + indiRes
						+ "', Numero_Civico='" + civ + "', Città_di_Residenza='" + citta
						+ "', Regione_di_Residenza='" + reg + "', E_mail='" + email + "' WHERE CM='" + cm + "'");
				...
				if (i == 1) {
					...
					Cookie userID = new Cookie("userid", cm);
		...
		if (session.getAttribute("arbitro") != null) {
			...
			if (!k) {
				...
				int i = st.executeUpdate("update associato set Num_Telefono=" + telefono + ", Indirizzo='" + indiRes
						+ "', Numero_Civico='" + civ + "', Città_di_Residenza='" + citta
						+ "', Regione_di_Residenza='" + reg + "', E_mail='" + email + "' WHERE CM='" + cm + "'");
				int j = star.executeUpdate("update arbitro set Taglia_Divisa='" + div + "', Taglia_Scarpe=" + scar
						+ " where CM='" + cm + "'");
				rs = agg.executeQuery("select * from associato where CM='" + cm + "'");
